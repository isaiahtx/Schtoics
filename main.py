...
for i in recurring_events:
    ...
    f3.write(
        ...
        + str(quarter_starts + datetime.timedelta(days=num_days)).replace("-", "")
        + "T"
        + start
        + "\n"
    )
    ...
    f3.write(
        ...
        + str(quarter_ends + datetime.timedelta(days=1)).replace("-", "")
        + "T000000Z;BYDAY="
        + i.days
        + "\n"
    )
    ...
    for j in holidays:
        if weekdays[j.weekday()] in i.days:
            f3.write(
                "EXDATE;TZID=America/Los_Angeles:"
                + str(j).replace("-", "")
                + "T"
                + start
                + "\n"
...
try:
    ...
    include_academic_calendar = input(
        "Do you want to include the academic calendar events in the calendar file? If\n"
        "you have used this tool this schoolyear before, choose no. Otherwise, choose yes \n(Y/n): "
    ).lower()
    if (
        include_academic_calendar == "y"
        or include_academic_calendar == "yes"
        or include_academic_calendar == ""
    ):
        f3.write(
            academic_calendar
        )  # Write the entirety of the contents of academic_calendar to the file
        f3.close()  # Close the file, we're done!
    elif include_academic_calendar == "n" or include_academic_calendar == "no":
        f3.write(
            "END:VCALENDAR"
        )  # Only write the calendar ending statement to the file
        f3.close()  # Close the file, we're done!
    else:
        raise ValueError
    print(
...
print(
    str(len(recurring_events) + len(one_time_events))
    + " Unique Calendar Events Created:"
)
for i in recurring_events:
